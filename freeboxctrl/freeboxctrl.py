...
class FreeboxCtrl:
    ...
    def configuration_lan_browser(self):
        data = self.__authenticated_request('/api/v3/lan/browser/pub/')
        return data['result']

    def state_lan_host(self, ether):
        data = self.__authenticated_request('/api/v3/lan/browser/pub/' + ether + '/')
        return data['result']
